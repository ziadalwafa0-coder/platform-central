  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        {connections.map((conn) => {
          ...
          return (
            <div 
              key={conn.id} 
              className="bg-[#141432] border border-[#2a2a5c] p-5 rounded-2xl text-right space-y-4 flex flex-col justify-between transition-all hover:border-[#6366f1]/45"
            >
              <div>
                <div className="flex justify-between items-start border-b border-[#2a2a5c]/40 pb-3">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      ...
                      <span className="text-[9px] text-[#a5a5c8] block font-mono">{conn.baseUrl || "غير مهيأ"}</span>
                    </div>
                  </div>
                  <span className={`text-[9px] px-2.5 py-0.5 rounded-full font-black ${
                    isActive ? "bg-[#24C78E]/15 text-[#24C78E]" : "bg-[#a5a5c8]/10 text-[#a5a5c8]"
                  }`}>
                    {isActive ? "نشط ومفعل 🟢" : "غير مفعل ⚪"}
                  </span>
                </div>

                <div className="space-y-2 text-[11px] text-[#a5a5c8] mt-3">
                  ...
                  <div className="flex justify-between">
                    ...
                    <strong className="text-white">
                      {conn.last_sync_status === "COMPLETED" ? (
                        ...
                        <span className="text-[#a5a5c8]">في انتظار أول مزامنة</span>
                      )}
                    </strong>
                  </div>
                </div>
              ...
              <div className={`mt-2 p-3 rounded-xl border flex items-center justify-between transition-all ${
                !lastSuccessStr 
                  ? "bg-[#0a0a1a]/40 border-[#2a2a5c]/40 text-[#a5a5c8]/80"
                  : isOverdue 
      ...
      <div className="bg-[#141432] border border-[#2a2a5c] p-6 rounded-3xl space-y-6">
        ...
        {/* Form Selector Tabs */}
        <div className="flex bg-[#0a0a1a] p-1 rounded-xl max-w-sm">
          <button 
            type="button"
            onClick={() => {
            ...
            className={`flex-1 py-2 rounded-lg text-xs font-bold transition cursor-pointer text-center ${
              selectedFormPlatform === "safka" ? "bg-[#6366f1] text-white" : "text-[#a5a5c8] hover:text-white"
            }`}
          >
            منصة صفقة Safka EG
          </button>
          <button 
            ...
            className={`flex-1 py-2 rounded-lg text-xs font-bold transition cursor-pointer text-center ${
              selectedFormPlatform === "custom" ? "bg-[#6366f1] text-white" : "text-[#a5a5c8] hover:text-white"
            }`}
          >
            قناة كتالوج مخصصة
          </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 text-right">
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-bold text-[#a5a5c8]">عنوان خادم الـ API الرئيسي (Base URL)</label>
            <input 
              type="text" 
              value={formBaseUrl}
              onChange={(e) => setFormBaseUrl(e.target.value)}
              className="w-full h-11 bg-[#0a0a1a] border border-[#2a2a5c] rounded-xl px-4 text-xs font-mono text-white outline-none focus:border-[#6366f1]"
              placeholder="https://api.safka-eg.com"
            />
          </div>

          <div className="space-y-1.5">
            <label className="text-xs font-bold text-[#a5a5c8]">مسار جلب المنتجات (Products Endpoint)</label>
            <input 
              type="text" 
              value={formEndpoint}
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-bold text-[#a5a5c8]">اسم ترويسة الترخيص (API Key Header)</label>
            <input 
              type="text" 
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-bold text-[#a5a5c8]">مفتاح الـ API الخاص بك (Private API Key)</label>
            <div className="relative">
              <input 
                type="password" 
                value={formApiKey}
                onChange={(e) => setFormApiKey(e.target.value)}
                className="w-full h-11 bg-[#0a0a1a] border border-[#2a2a5c] rounded-xl pr-4 pl-10 text-xs font-mono text-white outline-none focus:border-[#6366f1] text-right"
                placeholder="••••••••••••••••••••••••••••••••"
              />
              <Lock className="w-4 h-4 text-[#a5a5c8] absolute left-3.5 top-3.5" />
            </div>
          </div>
        ...
        <div className="flex gap-4 pt-2 justify-end">
          <button
            ...
            disabled={testing}
            className="px-5 h-11 bg-[#1c1c47] hover:bg-[#2a2a5c] border border-[#2a2a5c] text-xs font-bold text-[#f5f5fa] rounded-xl transition cursor-pointer flex items-center gap-2"
          >
            {testing ? (
              <RefreshCw className="w-4 h-4 animate-spin text-[#a5a5c8]" />
            ) : (
          ...
          <button
            ...
            className="px-6 h-11 bg-[#6366f1] hover:bg-[#818cf8] text-xs font-black text-white rounded-xl transition cursor-pointer shadow-lg shadow-[#6366f1]/15 flex items-center gap-2"
          >
