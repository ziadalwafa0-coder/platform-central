...
export default function ProductAdsSection({ productId }: { productId: string }) {
  ...
  if (loading) {
    return (
      <div className="space-y-3 pt-4 border-t border-[#20324A]/40">
    ...
    <div className="space-y-3 pt-4 border-t border-[#20324A]/40">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-80 overflow-y-auto pr-1 scrollbar-none">
        {ads.map((match: any) => {
          ...
          return (
            <div key={match.id} className="bg-[#07111F]/50 border border-[#20324A] rounded-xl p-3 flex flex-col gap-2 hover:border-[#20324A]/80 transition">
              <div className="flex gap-3">
                {ad.image_url ? (
                  <img src={ad.image_url} alt="Ad" className="w-16 h-16 rounded-lg object-cover bg-[#12233A] shrink-0 border border-[#20324A]/50" referrerPolicy="no-referrer" />
                ) : (
                  <div className="w-16 h-16 rounded-lg bg-[#12233A] shrink-0 border border-[#20324A]/50 flex items-center justify-center">
                    <Megaphone className="w-6 h-6 text-[#9FB0C5]/40" />
                  </div>
                )}
                <div className="flex-1 min-w-0">
                  <h5 className="text-[11px] font-bold text-[#F4F7FB] truncate">{ad.advertiser_name || "معلن غير معروف"}</h5>
                  <p className="text-[10px] text-[#9FB0C5] mt-0.5 line-clamp-2">{ad.body_text || ad.headline || "لا يوجد نص للإعلان"}</p>
                </div>
              </div>

              {analysis && (
                <div className="mt-1 bg-[#12233A]/50 rounded-lg p-2 border border-[#20324A]/30 text-[9px] text-[#9FB0C5] flex flex-col gap-1">
                  {analysis.hook && (
                  ...
                  {analysis.marketing_angle && (
                    <div className="flex gap-1.5 items-start">
                      <span className="text-[#2F80FF] font-bold shrink-0">الزاوية:</span>
              ...
              <div className="flex justify-between items-center mt-auto pt-2 border-t border-[#20324A]/40">
                <span className="text-[9px] px-2 py-0.5 bg-[#20324A]/50 rounded text-[#9FB0C5] font-mono">
                ...
                {ad.external_ad_id && ad.source_platform === "meta" && (
                  <a 
                    ...
                    className="text-[10px] text-[#2F80FF] hover:text-[#4090FF] flex items-center gap-1 font-bold"
                  >
                    رؤية الإعلان
