...
export function AdsSpyHistoryTab({ onSelectHistory }: AdsSpyHistoryTabProps) {
  ...
  if (history.length === 0) {
    return (
      <div className="bg-[#0a0a1a]/60 border border-[#2a2a5c] rounded-3xl p-12 text-center space-y-3">
        <Package className="w-10 h-10 text-[#a5a5c8] opacity-50 mx-auto" />
        <h3 className="text-white font-bold text-sm">لا يوجد سجل للإعلانات</h3>
        <p className="text-xs text-[#a5a5c8]">سيتم حفظ نتائج الفحص هنا تلقائياً لسهولة الرجوع إليها.</p>
      </div>
    );
  }
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative">
            ...
            <input
              ...
              className="bg-[#0f0f24] border border-[#2a2a5c] text-white text-xs rounded-xl pl-4 pr-9 py-2 focus:outline-none focus:border-[#8B5CF6] transition-all w-64"
            />
          </div>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredHistory.map((entry) => (
          <div 
            ...
            className="bg-[#0a0a1a]/60 border border-[#2a2a5c] hover:border-[#8B5CF6]/50 rounded-2xl p-4 cursor-pointer transition-all hover:shadow-lg hover:shadow-[#8B5CF6]/10 flex flex-col gap-3 group relative"
          >
            <button 
            ...
            <div className="flex items-start gap-3">
              {entry.product.image_url ? (
                <img src={entry.product.image_url} alt={entry.product.name} className="w-12 h-12 rounded-xl object-cover border border-[#2a2a5c]" />
              ) : (
                <div className="w-12 h-12 rounded-xl bg-[#0f0f24] border border-[#2a2a5c] flex items-center justify-center">
                  <Package className="w-5 h-5 text-[#a5a5c8]" />
                </div>
              )}
              <div className="flex-1 min-w-0">
                <h4 className="text-white font-bold text-sm truncate pr-6">{entry.product.name}</h4>
                {entry.product.sku && <p className="text-[#a5a5c8] text-[10px] font-mono mt-0.5">SKU: {entry.product.sku}</p>}
              </div>
            </div>

            <div className="grid grid-cols-2 gap-2 mt-auto">
              <div className="bg-[#0f0f24] rounded-lg p-2 flex flex-col items-center justify-center">
                <span className="text-[10px] text-[#a5a5c8]">الإعلانات المحفوظة</span>
                <span className="text-white font-bold text-sm">{entry.matches.length}</span>
              </div>
              <div className="bg-[#0f0f24] rounded-lg p-2 flex flex-col items-center justify-center">
                <span className="text-[10px] text-[#a5a5c8]">تاريخ الفحص</span>
